...
<script>
  import { mailboxVerification2, interfaceAddUser } from '../assets/js/api'
  export default {
    name: 'enrollUser',
    data() {
      return {
    ...
    methods: {
      ...
      gitVerification() {
        if(/^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/.test(this.ruleForm.email)){
          this.countdown(59)
          mailboxVerification2(this, this.ruleForm.email)
        } else {
    ...
    created () {
      if(localStorage.getItem("enrollUserWait") > 1) {
        this.yzmbr = true
        this.yzmHtml = '重新发送(' + localStorage.getItem("enrollUserWait") + ')'
        this.countdown(localStorage.getItem("enrollUserWait"))
      }
    },
    beforeDestroy () {
...
<style scoped>
  .login{height:100vh;background:url(../assets/images/beiji.jpg) no-repeat}
