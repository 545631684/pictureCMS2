...
<script>
	...
	import { deleteTemporaryFile, addImgsFile, queryTitle, deleteTemporaryFile2, addImgsFileText } from '../assets/js/api'
	...
	export default {
		...
		methods: {
			...
			obtainImgSrc(file, fileList) {
				...
				if(file.response !== undefined) {
					if(file.response.type === '1') {
						...
						this.uploadFiles.push(file.response.dataVideo, file.response.Videoview)
						// 设置标题为第一个上传的文件的名称
						if (this.title.length === 0) {
							this.title = this.video[0].dataVideo.name.split(".")[0]
							this.titleRepeat()
						}
					}
			...
			handleRemove3(file, fileList) {
				...
				this.setuploadFiles(file.response.Videoview)
			...
			submitImg() { 
				if (this.$store.state.user.nickname === this.$store.state.user.userName) {
					...
					this.loading = true
					let img = this.imgCrss.length > 0 && this.imgCrss !== '[]' ? this.imgCrss : '[]'
					addImgsFile(this, this.$store.getters.getUserTypesId(this.typeImg).tid, this.$store.getters.getUserProjectsId(this.projectImg).pid, img, '[]', this.video, this.describe, this.$store.getters.getUserMinTypeId(this.minTypeImg).did, 'video')
				}
			},
			// 判断标题是否重复，并提示用户
			titleRepeat() {
